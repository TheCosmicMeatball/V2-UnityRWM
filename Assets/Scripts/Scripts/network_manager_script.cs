...
public class RWMNetworkManager : NetworkBehaviour
{
    ...
    {
        RelayInternet,     // Unity Relay over the internet (and also works on same machine)
        LocalDirectDebug   // Direct connection to IP/localhost (for debugging)
    }

    [Header("Network Mode")]
    public NetworkMode networkMode = NetworkMode.RelayInternet;
...
#pragma warning restore CS0067
    ...
    private void Awake()
    {
        ...
        // WebGL always uses WebSockets under the hood; enable flag early to silence warnings
        if (Application.platform == RuntimePlatform.WebGLPlayer)
        {
            try
            {
                if (unityTransport == null)
                {
                    ...
                    if (nm != null)
                    {
                        unityTransport = nm.GetComponent<UnityTransport>();
                    }
                }
                if (unityTransport != null)
                {
        ...
        // Make sure we have NetworkManager, NetworkObject, UnityTransport, etc.
        ...
        // Make sure Netcode uses this transport
        if (networkManager.NetworkConfig.NetworkTransport != unityTransport)
        {
            networkManager.NetworkConfig.NetworkTransport = unityTransport;
        }

        // For our game, we want WebSockets available everywhere (WebGL requires it,
        // desktop can still talk to Relay over secure transport).
        if (unityTransport != null)
        {
            unityTransport.UseWebSockets = true;
            Debug.Log($"[NetworkManager] UnityTransport.UseWebSockets = {unityTransport.UseWebSockets} on {Application.platform}");
        }

        // IMPORTANT:
        // Only configure raw IP/port and reapply on scene load in LocalDirectDebug mode.
        // In RelayInternet mode, RelayAdapter.SetRelayServerData() owns the transport config.
        if (networkMode == NetworkMode.LocalDirectDebug)
        {
            ...
            Debug.Log($"[NetworkManager] Applied LOCAL DIRECT transport config at Awake() -> {advertisedAddress}:{port} bind={bindAddress}");

            // Re-apply local direct settings after scene loads (for non-Relay mode only)
            UnityEngine.SceneManagement.SceneManager.sceneLoaded += (scene, mode) =>
            {
                if (unityTransport == null)
                {
                ...
                unityTransport.SetConnectionData(advertisedAddress, port, bindAddress);
                Debug.Log($"[NetworkManager] Reapplied LOCAL DIRECT transport settings in scene '{scene.name}'");
            };
        }
    }
    ...
    private async void TryStartHostRelayOrLocal()
    {
        if (!EnsureNetworkingComponents())
        {
            Debug.LogError("[NetworkManager] TryStartHostRelayOrLocal: Networking components not ready.");
            OnConnectionError?.Invoke();
            return;
        }
        ...
        switch (networkMode)
        {
            case NetworkMode.RelayInternet:
                await StartRelayHostAsync();
                break;

            case NetworkMode.LocalDirectDebug:
                StartLocalHost();
                break;
        }
    }
    
    //new helper methodss by ali
    // === RELAY HOST (INTERNET) ===
    private async Task StartRelayHostAsync()
    {
        Debug.Log("[NetworkManager] Starting Relay host...");

        var result = await RelayAdapter.StartHostAsync(networkManager, unityTransport, maxConnections: 16);
        if (!result.ok)
        {
            Debug.LogError($"[NetworkManager] Relay host failed: {result.error}");
            OnConnectionError?.Invoke();
            return;
        }

        // Relay join code becomes our room code
        roomCode = result.joinCode;
        Debug.Log($"[NetworkManager] Relay host started. JoinCode (roomCode): {roomCode}");

        try { OnRoomCreated?.Invoke(roomCode); } catch { }
    }

    // === LOCAL DIRECT HOST (DEBUG) ===
    private void StartLocalHost()
    {
        Debug.Log("[NetworkManager] Starting LOCAL DIRECT host...");

        // UI-only code – networking uses IP/port
        GenerateRoomCode();
        ...
        unityTransport.SetConnectionData(advertisedAddress, port, bindAddress);
        Debug.Log($"[NetworkManager] Local host binding {bindAddress} advertising {advertisedAddress}:{port}");

        bool success = networkManager.StartHost();
        if (success)
        {
            Debug.Log($"[NetworkManager] Host started (LocalDirectDebug). Room code: {roomCode}");
            try { OnRoomCreated?.Invoke(roomCode); } catch { }
        }
        else
        {
            Debug.LogError("[NetworkManager] Failed to start host (LocalDirectDebug)");
            OnConnectionError?.Invoke();
        }
    }
    ...
    public bool JoinGame(string code, string hostIP = "127.0.0.1")
    {
        ...
        {
            case NetworkMode.RelayInternet:
                TryJoinRelayDirect(roomCode);
                return true; // async path handles result

            case NetworkMode.LocalDirectDebug:
                return StartLocalClient(roomCode, hostIP);
    ...
    private async void TryJoinRelayDirect(string relayCode)
    {
        Debug.Log($"[NetworkManager] Attempting Relay join with code: {relayCode}");

        var result = await RelayAdapter.JoinAsync(networkManager, unityTransport, relayCode);
        if (!result.ok)
        {
        ...
        Debug.Log("[NetworkManager] Relay client started successfully.");
    }

    private bool StartLocalClient(string code, string hostIP)
    {
        ...
        Debug.Log($"[NetworkManager] Starting LOCAL DIRECT client to {hostIP}:{port} roomCode={code}");
        ...
        if (success)
        {
            Debug.Log($"[NetworkManager] Client connecting to {hostIP}:{port}");
        }
        else
        {
            Debug.LogError("[NetworkManager] Failed to start client (LocalDirectDebug)");
            OnConnectionError?.Invoke();
        }
